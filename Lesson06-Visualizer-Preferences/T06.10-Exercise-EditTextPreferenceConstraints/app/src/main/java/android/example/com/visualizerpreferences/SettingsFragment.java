...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        Preference preference = findPreference(getResources().getString(R.string.pref_size_key));
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        Toast toast = Toast.makeText(getActivity(), "Bla bla bla messages ...", Toast.LENGTH_LONG);

        String preferenceKey = getResources().getString(R.string.pref_size_key);
        if (preference.getKey().equals(preferenceKey)) {
            try {
                float size = Float.parseFloat(String.valueOf(newValue));
                if (size <= 0 || size > 3) {
                    toast.show();
                    return false;
                }
